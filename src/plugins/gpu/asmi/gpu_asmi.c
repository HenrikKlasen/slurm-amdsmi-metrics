...
/*****************************************************************************\
 *  gpu_asmi.c - Support amd-smi interface to an AMD GPU.
 *****************************************************************************
 *  Copyright (C) SchedMD LLC.
 *  Copyright (c) 2019, Advanced Micro Devices, Inc. All rights reserved.
 *  Copyright (c) 2026, Unniversity of Luxembourg
 *  Written by Advanced Micro Devices,
 *  Modified by Henrik Klasen, University of Luxembourg
...
const char plugin_name[] = "GPU AMD-SMI plugin";
const char plugin_type[] = "gpu/amd-smi";
...
static void _rsmi_init()
{
	...
	END_TIMER;
	debug3("amdsmi_init() took %s", TIMER_STR());
	if (amdsmi_rc != AMDSMI_STATUS_SUCCESS) {
		amdsmi_status_string(amdsmi_rc, &status_string);
		error("Failed to initialize amdsmi: %s",
		      status_string);
	} else
		debug2("Successfully initialized amdsmi");
	...
	debug("AMD-SMI Library Version: %s", version);
}
				...
				uint32_t *mem_freqs)
{
	...
	amdsmi_processor_handle_t *handles = NULL;
	uint32_t count;

	amdsmi_rc = amdsmi_get_processor_handles(AMDSMI_INIT_AMD_GPUS, &handles, &count);
	if (amdsmi_rc != AMDSMI_STATUS_SUCCESS || dv_ind >= count) {
		error("AMD-SMI: Failed to get handles or invalid dv_ind %u", dv_ind);
		return false;
	}

	DEF_TIMERS;
	START_TIMER;
	amdsmi_rc = amdsmi_get_clk_freq(
		handles[dv_ind], AMDSMI_CLK_TYPE_MEM, &amdsmi_freqs);
	END_TIMER;
	debug3("amdsmi_get_clk_freq() took %s",
	       TIMER_STR());

	if (amdsmi_rc != AMDSMI_STATUS_SUCCESS) {
		amdsmi_rc = amdsmi_status_string(amdsmi_rc, &status_string);
		error("AMD-SMI: Failed to get memory frequencies error: %s",
		      status_string);
		return false;
	}
				...
				uint32_t *gfx_freqs)
{
	...
	START_TIMER;
	amdsmi_rc = amdsmi_get_clk_freq(
		handles[dv_ind], AMDSMI_CLK_TYPE_GFX, &amdsmi_freqs);
	END_TIMER;
	debug3("amdsmi_get_clk_freq() took %s", TIMER_STR());

	if (amdsmi_rc != AMDSMI_STATUS_SUCCESS) {
		amdsmi_rc = amdsmi_status_string(amdsmi_rc, &status_string);
		error("AMD-SMI: Failed to get graphics frequencies error: %s",
		      status_string);
		return false;
	}
			    ...
			    uint64_t gfx_bitmask)
{
	...
	DEF_TIMERS;
	START_TIMER;
	amdsmi_rc = amdsmi_set_clk_freq(
		handles[dv_ind], AMDSMI_CLK_TYPE_MEM, mem_bitmask);
	END_TIMER;
	debug3("amdsmi_set_clk_freq(0x%lx) for memory took %s",
	       mem_bitmask, TIMER_STR());
	if (amdsmi_rc != AMDSMI_STATUS_SUCCESS) {
		amdsmi_rc = amdsmi_status_string(amdsmi_rc, &status_string);
		error("AMD-SMI: Failed to set memory frequency GPU %u error: %s",
		      dv_ind, status_string);
		return false;
	}

	START_TIMER;
	amdsmi_rc = amdsmi_set_clk_freq(handles[dv_ind],
					    AMDSMI_CLK_TYPE_GFX, gfx_bitmask);
	debug3("amdsmi_set_clk_freq(0x%lx) for graphics took %s",
	       gfx_bitmask, TIMER_STR());
	END_TIMER;
	if (amdsmi_rc != AMDSMI_STATUS_SUCCESS) {
		amdsmi_rc = amdsmi_status_string(amdsmi_rc, &status_string);
		error("AMD-SMI: Failed to set graphic frequency GPU %u error: %s",
		      dv_ind, status_string);
		return false;
	}
	return true;
...
static bool _rsmi_reset_freqs(uint32_t dv_ind)
{
	...
	START_TIMER;
	amdsmi_rc = amdsmi_set_gpu_perf_level(handles[dv_ind], AMDSMI_DEV_PERF_LEVEL_AUTO);
	END_TIMER;
	debug3("amdsmi_set_gpu_perf_level() took %s", TIMER_STR());
	if (amdsmi_rc != AMDSMI_STATUS_SUCCESS) {
		amdsmi_rc = amdsmi_status_string(amdsmi_rc, &status_string);
		error("AMD-SMI: Failed to reset frequencies error: %s",
		      status_string);
		return false;
	}
	return true;
 ...
 */
static uint32_t _rsmi_get_freq(amdsmi_processor_handle_t handle, amdsmi_clk_type_t type)
{
	const char *status_string;
	amdsmi_status_t amdsmi_rc;
	amdsmi_frequencies_t amdsmi_freqs;
	...
	START_TIMER;
	amdsmi_rc = amdsmi_get_clk_freq(handle, type, &amdsmi_freqs);
	END_TIMER;
	debug3("amdsmi_get_clk_freq(%s) took %s", type_str, TIMER_STR());
	if (amdsmi_rc != AMDSMI_STATUS_SUCCESS) {
		amdsmi_rc = amdsmi_status_string(amdsmi_rc, &status_string);
		error("AMD-SMI: Failed to get the GPU frequency type %s, error: %s",
...
static uint32_t _rsmi_get_gfx_freq(amdsmi_processor_handle_t handle)
{
	return _rsmi_get_freq(handle, AMDSMI_CLK_TYPE_GFX);
}

static uint32_t _rsmi_get_mem_freq(amdsmi_processor_handle_t handle)
{
	return _rsmi_get_freq(handle, AMDSMI_CLK_TYPE_MEM);
}
...
static void _reset_freq(bitstr_t *gpus)
{
	...
	amdsmi_rc = amdsmi_get_processor_handles(&handles, &num_handles);
	if (amdsmi_rc != AMDSMI_STATUS_SUCCESS) {
		amdsmi_rc = amdsmi_status_string(amdsmi_rc, &status_string);
		error("AMD-SMI: Failed to get processor handles, error: %s", status_string);
		return;
	}
	...
	for (i = 0; i < gpu_len; i++) {
		...
		debug2("Memory frequency before reset: %u",
		       _rsmi_get_mem_freq(handles[i]));
		debug2("Graphics frequency before reset: %u",
		       _rsmi_get_gfx_freq(handles[i]));
...
static void _rsmi_get_driver(char *driver, unsigned int len)
{
	amdsmi_get_gpu_driver_info(0, driver, len);  // Assuming device 0 or global
}
...
static void _rsmi_get_version(char *version, unsigned int len)
{
	...
	amdsmi_status_t amdsmi_rc = amdsmi_get_lib_version(&amdsmi_version);

	if (amdsmi_rc != AMDSMI_STATUS_SUCCESS) {
		amdsmi_rc = amdsmi_status_string(amdsmi_rc, &status_string);
		error("AMD-SMI: Failed to get the version error: %s",
		      status_string);
		version[0] = '\0';
	} else {
		snprintf(version, len, "%s", amdsmi_version.build);
		if (amdsmi_version.major < RSMI_REQ_VERSION_USAGE) {
			get_usage = false;
			error("%s: GPU usage accounting disabled. AMD-SMI version >= 6.0.0 required.",
			      __func__);
		}
	}
}
...
{
	amdsmi_processor_handle_t *handles = NULL;
	amdsmi_status_t amdsmi_rc = amdsmi_get_processor_handles(AMDSMI_INIT_AMD_GPUS, &handles, device_count);

	if (amdsmi_rc != AMDSMI_STATUS_SUCCESS) {
		const char *status_string;
		amdsmi_rc = amdsmi_status_string(amdsmi_rc, &status_string);
		error("AMD-SMI: Failed to get device count: %s", status_string);
		*device_count = 0;
	}
	// Free handles if allocated
				  ...
				  unsigned int size)
{
	...
	amdsmi_status_t amdsmi_rc = amdsmi_get_gpu_asic_info(dv_ind, &info);

	if (amdsmi_rc != AMDSMI_STATUS_SUCCESS) {
		amdsmi_rc = amdsmi_status_string(amdsmi_rc, &status_string);
		error("AMD-SMI: Failed to get name of the GPU: %s", status_string);
	} else {
		strncpy(device_name, info.market_name, size);  // Assuming market_name is the field
	}
				   ...
				   unsigned int size)
{
	...
	if (amdsmi_rc != AMDSMI_STATUS_SUCCESS) {
		amdsmi_rc = amdsmi_status_string(amdsmi_rc, &status_string);
		error("AMD-SMI: Failed to get brand of the GPU: %s",
		      status_string);
	} else {
		strncpy(device_brand, info.vendor_name, size);  // Assuming vendor_name
	}
					  ...
					  unsigned int *minor)
{
	const char *status_string;
	amdsmi_bdf_t bdf;
	amdsmi_status_t amdsmi_rc = amdsmi_get_gpu_device_bdf(dv_ind, &bdf);

	if (amdsmi_rc != AMDSMI_STATUS_SUCCESS) {
		amdsmi_rc = amdsmi_status_string(amdsmi_rc, &status_string);
		error("AMD-SMI: Failed to get minor number of GPU: %s",
		      status_string);
	} else {
		*minor = bdf.minor;  // Assuming bdf has minor
	}
}
 ...
 */
static void _rsmi_get_device_pci_info(uint32_t dv_ind, amdsmiPciInfo_t *pci)
{
	const char *status_string;
	amdsmi_bdf_t bdf;
	amdsmi_status_t amdsmi_rc = amdsmi_get_gpu_device_bdf(dv_ind, &bdf);
	...
	if (amdsmi_rc != AMDSMI_STATUS_SUCCESS) {
		amdsmi_rc = amdsmi_status_string(amdsmi_rc, &status_string);
		error("AMD-SMI: Failed to get PCI Info of the GPU: %s",
		      status_string);
	} else {
		// Construct bdfid from bdf
		pci->bdfid = (bdf.domain << 32) | (bdf.bus << 16) | (bdf.device << 8) | bdf.function;
	}
}
...
{
	const char *status_string;
	amdsmi_status_t amdsmi_rc = amdsmi_get_gpu_id(dv_ind, id);

	if (amdsmi_rc != AMDSMI_STATUS_SUCCESS) {
		amdsmi_rc = amdsmi_status_string(amdsmi_rc, &status_string);
		error("AMD-SMI: Failed to get Unique ID of the GPU: %s",
		      status_string);
	}
}
...
static bitstr_t *_rsmi_get_device_cpu_mask(uint32_t dv_ind)
{
	...
	amdsmi_status_t amdsmi_rc =	amdsmi_get_gpu_topo_numa_affinity(dv_ind, &nnid);

	if (amdsmi_rc != AMDSMI_STATUS_SUCCESS) {
		const char *status_string;
		amdsmi_rc = amdsmi_status_string(amdsmi_rc, &status_string);
		error("AMD-SMI: Failed to get numa affinity of the GPU: %s",
		      status_string);
		return NULL;
	}
...
extern int gpu_p_energy_read(uint32_t dv_ind, gpu_status_t *gpu)
{
	...
	amdsmi_rc = amdsmi_get_processor_handles(AMDSMI_INIT_AMD_GPUS, &handles, &count);
	if (amdsmi_rc != AMDSMI_STATUS_SUCCESS || dv_ind >= count) {
		error("AMD-SMI: Failed to get processor handles or invalid dv_ind %u", dv_ind);
		gpu->energy.current_watts = NO_VAL;
		return SLURM_ERROR;
	}

	amdsmi_rc = amdsmi_get_power_info(handles[dv_ind], &power_info);

	if (amdsmi_rc != AMDSMI_STATUS_SUCCESS) {
		amdsmi_status_string(amdsmi_rc, &status_string);
		error("AMD-SMI: Failed to get power: %s", status_string);
		gpu->energy.current_watts = NO_VAL;
		return SLURM_ERROR;
	}

	gpu->last_update_watt = power_info.average / 1000000;
...
extern int gpu_p_usage_read(pid_t pid, acct_gather_data_t *data)
{
	 ...
	 */
	if (!get_usage) {
		debug2("%s: AMD-SMI version check for usage gathering.", __func__);
		return SLURM_SUCCESS;
	}

	rc = amdsmi_get_gpu_compute_process_info_by_pid(pid, &proc);
	...
	if (rc == AMDSMI_STATUS_NOT_FOUND) {
		...
		error("AMD-SMI: Failed to get usage(%d): %s", rc, status_string);
		return SLURM_ERROR;
	}
